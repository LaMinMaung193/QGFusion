...
**Phases 0–3: COMPLETE**  
Engineering foundation is fully in place. All 4 loss terms converge on fixed samples.
The model can memorize — the scientific validation phases begin now.

**Next step: Phase 4 — Full-Scale Training**
...
This is engineering, not science — but it must work cleanly before evaluation.

### 4A — Mixed precision + memory audit
- [ ] Enable `torch.cuda.amp` in `train.py` (close to mandatory on single 3090)
- [ ] Profile peak VRAM at batch_size=1 with AMP — target under 20GB
- [ ] If OOM: gradient accumulation (steps=2) before reducing model size
- [ ] Verify loss and Gaussian health unchanged after enabling AMP

### 4B — Backbone upgrade (important for paper-quality numbers)
Current ResNet-50 + ImageNet is a placeholder. Before submitting results, upgrade:
- [ ] Camera: swap to **DINOv2-Small** (ViT-S/14, ~22M params) or keep ResNet-50 for speed
  - DINOv2 gives meaningfully better semantic features; cite as design choice in paper
- [ ] LiDAR: switch config to `lidar_backend: spconv` (already implemented, currently set to `dense`)
  - Note: spconv backend adds ~50ms/step but gives proper sparse 3D features
- [ ] Radar: current MLP is fine for V1; upgrade path is BEV radar encoder if numbers disappoint

### 4C — Full mini_train run
- [ ] Train 50–100 epochs on nuScenes-mini (323 train / 81 val samples)
- [ ] Log per-head losses separately in wandb, not just total
- [ ] Monitor Gaussian health every 50 steps using `overfit_test.py` monitor logic
  (add it to `train.py` as a lightweight periodic check)
- [ ] Exit criteria: all loss terms trending down, no collapse, val loss not exploding

### 4D — Scale to full nuScenes trainval (if compute allows)
- [ ] nuScenes trainval symlink consolidation script (noted in Phase 1: blob folders 01–05)
- [ ] Train 24–48 epochs on full trainval
- [ ] Save checkpoint every 4 epochs (not every 1 — saves disk space)
...
**Exit criteria:** checkpoint trained on at least nuScenes-mini with stable loss curves
for all three heads over 50+ epochs.
...
Track things that need an explicit decision rather than a silent default:

| # | Decision | Status | Notes |
|---|----------|--------|-------|
| 1 | Camera backbone: ResNet-50 vs DINOv2 | Open | DINOv2 gives better numbers; ResNet-50 trains faster |
| 2 | LiDAR backend: dense vs spconv | Open | spconv is correct choice; switch before Phase 5 |
| 3 | Full trainval vs mini only | Open | Depends on time budget; mini is honest if framed correctly |
| 4 | Box convention (x,y,z,w,l,h,sin,cos) vs devkit format | Open | Verify before Phase 5B |
| 5 | Background class in DetectionHead | Open | Required for honest mAP; implement in Phase 5B |
| 6 | Target venue and deadline | Open | Discuss with Prof. Chiang after Phase 5 |
| 7 | Number of ablation seeds | Open | 1 seed is minimum; 2–3 strengthens claims |

---

## Key Notes for the Paper

Things to keep in mind throughout — record these as you go:
...
- Training time per epoch (ms/step)
- Peak GPU memory usage
- Model parameter count (`sum(p.numel() for p in model.parameters())`)
- Convergence epoch for each loss term
...
|-------------|-----------|----------------|
| Phase 3 (now) | 0% | No evaluation results |
| Phase 4 | 20% | Training runs but no numbers |
